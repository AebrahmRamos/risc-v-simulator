...
export default function ToolBar({ runState, onRun, onPause, onStep, onReset, onAssemble, assemblerErrorsCount }: Props) {
  return (
    <div className="flex items-center gap-3 p-2 bg-[#1f2335] border-b border-[#1f2335]">
      <button className="px-3 py-1 bg-[#9ece6a] text-[#1a1b26] rounded text-sm font-medium hover:bg-[#b9f27c]" onClick={onRun}>Run</button>
      <button className="px-3 py-1 bg-[#e0af68] text-[#1a1b26] rounded text-sm font-medium hover:bg-[#f0bf78]" onClick={onPause}>Pause</button>
      <button className="px-3 py-1 bg-[#7aa2f7] text-[#1a1b26] rounded text-sm font-medium hover:bg-[#8ab2ff]" onClick={onStep}>Step</button>
